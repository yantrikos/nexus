/**
 * Nexus — Durable Event Bus Plugin for OpenClaw
 *
...
const BRIDGE_SCRIPT = path.join(PLUGIN_DIR, "nexus_engine", "bridge.py");
...
export async function onStartup(context: {
  ...
  try {
    ...
    if (result.healthy) {
      ...
      console.log(
        `[nexus] Event bus ready: ${stats.total_events ?? 0} events, ${stats.consumer_groups ?? 0} groups`
      );
    } else {
      console.error(`[nexus] Health check failed: ${result.error}`);
    }
  } catch (err) {
    console.error(`[nexus] Startup error: ${(err as Error).message}`);
  }
}

export async function onShutdown(): Promise<void> {
  killProcess();
  console.log("[nexus] Event bus shut down");
}
